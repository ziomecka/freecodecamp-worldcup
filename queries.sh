...
echo  "$($PSQL "SELECT DISTINCT name FROM ((SELECT name FROM games LEFT JOIN teams ON games.winner_id=teams.team_id WHERE year=2014 AND round='Eighth-Final') UNION (SELECT name FROM games LEFT JOIN teams ON games.opponent_id=teams.team_id WHERE year=2014 AND round='Eighth-Final') )as teams")"
